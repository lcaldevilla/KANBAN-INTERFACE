...
class ConsumosController extends Controller
{
    ...
    public function ajaxRequestPostUpdate(Request $request)
    {
        ...
        $consumos = DB::table('PEDIDO_REPOSICION as pr')
            ...
            //->select('pr.pedido_repo_id',  'pr.Fecha_creacion', 'pr.usuario_id', 'u.dsc_usuario', 'prd.fecha',  'prd.codigo_urgencia', 'prd.cant_reponer', 'prd.cod_servicio', 'prd.cod_almacen', 'prd.cod_producto', 'p.dsc_producto')
        ...
        return view('consumos_pagination', compact('consumos', 'total_Registros', 'total_articulos'))->render();
    ...
    public function index()
    {
        ...
        // adapto la fecha
        $auxDesde = Carbon::parse($desde)->format('d-m-Y');
        $auxHasta = Carbon::parse($hasta)->format('d-m-Y');
        error_log("FECHA DESDE: ".$auxDesde);
        // añado las fechas en la cláusula WHERE
        array_push($aux_array_desde, 'pr.fecha_creacion', '>=', $auxDesde);
        array_push($matches, $aux_array_desde);
        ...
        return view('consumos', compact('consumos', 'servicios', 'almacenes', 'total_Registros', 'total_articulos'))->render();
    }
}

...
class consumosAgrupadosUrgentesController extends Controller
{
    ...
    public function ajaxRequestPostUpdate(Request $request)
    {
        ...
        $consumos = DB::table('PEDIDO_REPOSICION as pr')
            ...
            ->select('prd.codigo_urgencia', DB::raw('sum(convert(int, prd.cant_reponer)) as total'), 'prd.cod_servicio', 'prd.cod_almacen', 'prd.cod_producto', 'p.dsc_producto')
            ->where($matches)
            ->groupBy('prd.cod_servicio', 'prd.cod_almacen', 'prd.cod_producto', 'p.dsc_producto',  'prd.cant_reponer','prd.codigo_urgencia')
            ->get();
        ...
        //if(count($total_articulos) == 0) $total_articulos = 0;
        $total_Registros = count($consumos);


        // error_log("update view: ".count($total_articulos));
        return view('consumos_urgentes_agrupados_pagination', compact('consumos', 'total_Registros', 'total_articulos'))->render();
    ...
    public function index()
    {
        ...
        $consumos = DB::table('PEDIDO_REPOSICION as pr')
            ->join('PEDIDO_REPOSICION_DET as prd', 'pr.pedido_repo_id', '=', 'prd.pedido_repo_id')
            ->join('producto as p', 'p.cod_producto', '=', 'prd.cod_producto')
            ->select( DB::raw('sum(convert(int, prd.cant_reponer)) as total'), 'prd.cod_servicio', 'prd.cod_almacen', 'prd.cod_producto', 'p.dsc_producto')
            ->where($matches)
            ->groupBy('prd.cod_servicio', 'prd.cod_almacen', 'prd.cod_producto', 'p.dsc_producto', 'prd.cant_reponer')
            ->get();
        ...
        return view('consumos_agrupados_urgentes', compact('consumos', 'servicios', 'almacenes', 'total_Registros', 'total_articulos'))->render();
    }
}
